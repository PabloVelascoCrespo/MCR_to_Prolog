...
#TODO: Controlar que da error de timeout :D
def traductor(synset):
    print(synset)
    cadena = ""
    try:
        cadena = translator.translate(dfEng.loc[dfEng['Synset'] == synset].Glosa.item()).text
    except:
        cadena = dfEng.loc[dfEng['Synset'] == synset].Glosa.item()
    return cadena
